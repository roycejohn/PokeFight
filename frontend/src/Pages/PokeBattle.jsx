...
function PokeBattle() {
  ...
  const [battleOver, setBattleOver] = useState(false); // New state to track if battle is over
  ...
  const startBattle = (pokemon1, pokemon2) => {
    ...
    const battleRound = () => {
      if (battleOver) return; // Check if battle is over before proceeding
      ...
      if (hp2 <= 0) {
        ...
        setBattleLog(log);
        setBattleOver(true); // Set battle over flag
        return;
      }
      ...
      setBattleLog(log);
      setTimeout(battleRound, 500); // Delay between rounds (500ms = 0.5 second)
    };
